...
class DGITrainer(Trainer):
    ...
    def predict(self, batch):
        ...
        Feature = torch.cat((User_feature, Item_feature),
                            dim=-1)
        output = self.gen.score_predict(Feature)
    ...
    def reconstruct(self, UV, VU, adj, CUV, CVU, fake_adj, user_feature, item_feature, batch):
        ...
        pos_One = self.gen.score(torch.cat((user_feature_Two, item_feature_Two), dim=1))
        neg_One = self.gen.score(torch.cat((user_feature_Two, neg_item_feature_Two), dim=1)) 
